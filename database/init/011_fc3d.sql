-- Multi-lottery support: SSQ + FC3D (福彩3D)
-- Additive only — existing SSQ rows default to SSQ.

ALTER TABLE lottery_history
    ADD COLUMN lottery_type VARCHAR(16) NOT NULL DEFAULT 'SSQ' COMMENT 'SSQ | FC3D' AFTER period,
    ADD INDEX idx_history_lottery_type (lottery_type);

ALTER TABLE predict_record
    ADD COLUMN lottery_type VARCHAR(16) NOT NULL DEFAULT 'SSQ' COMMENT 'SSQ | FC3D' AFTER period,
    ADD INDEX idx_predict_lottery_type (lottery_type);

ALTER TABLE backtest_report
    ADD COLUMN lottery_type VARCHAR(16) NOT NULL DEFAULT 'SSQ' COMMENT 'SSQ | FC3D' AFTER name;

-- FC3D_DRAW_RECORD
CREATE TABLE IF NOT EXISTS fc3d_draw_record (
    id                BIGINT AUTO_INCREMENT PRIMARY KEY,
    issue             VARCHAR(32)  NOT NULL UNIQUE COMMENT '期号',
    digit1            TINYINT      NOT NULL COMMENT '百位 0-9',
    digit2            TINYINT      NOT NULL COMMENT '十位 0-9',
    digit3            TINYINT      NOT NULL COMMENT '个位 0-9',
    sum_value         INT          NOT NULL COMMENT '和值',
    span_value        INT          NOT NULL COMMENT '跨度',
    odd_even_pattern  VARCHAR(16)  NOT NULL COMMENT '奇偶形态 O/E',
...
-- Sample FC3D draws for local dev
